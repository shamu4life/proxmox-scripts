...
if whiptail --backtitle "Proxmox VE Helper Scripts" --title "Docker VM with Dockge" --yesno "This will create a New Docker VM with Dockge pre-installed. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${CREATING}${BOLD}${DGN}Creating a Docker VM with Dockge using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "SETTINGS" --yesno "Ready to create a Docker VM with Dockge?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a Docker VM with Dockge using the above advanced settings${CL}"
  else
    header_info
...
cat <<EOF >"$TEMP_DIR/dockge-startup.sh"
#!/bin/bash
cd /opt/stacks/dockge
/usr/bin/docker compose up -d
systemctl disable dockge-startup.service
EOF

cat <<EOF >"$TEMP_DIR/dockge-startup.service"
[Unit]
Description=Initial Dockge Stack Startup
After=docker.service network-online.target
Requires=docker.service network-online.target
[Service]
Type=oneshot
ExecStart=/usr/local/bin/dockge-startup.sh
[Install]
WantedBy=multi-user.target
EOF
msg_ok "Created Dockge configuration files successfully"
...
virt-customize -q -a "${FILE}" --install qemu-guest-agent,apt-transport-https,ca-certificates,curl,gnupg,lsb-release >/dev/null &&
  ...
  virt-customize -q -a "${FILE}" --upload "$TEMP_DIR/docker-compose.yml:/opt/stacks/dockge/docker-compose.yml" >/dev/null &&
  virt-customize -q -a "${FILE}" --upload "$TEMP_DIR/.env:/opt/stacks/dockge/.env" >/dev/null &&
  virt-customize -q -a "${FILE}" --upload "$TEMP_DIR/dockge-startup.sh:/usr/local/bin/dockge-startup.sh" >/dev/null &&
  virt-customize -q -a "${FILE}" --upload "$TEMP_DIR/dockge-startup.service:/etc/systemd/system/dockge-startup.service" >/dev/null &&
  virt-customize -q -a "${FILE}" --chmod "0755:/usr/local/bin/dockge-startup.sh" >/dev/null &&
  virt-customize -q -a "${FILE}" --run-command "systemctl enable dockge-startup.service" >/dev/null &&
  virt-customize -q -a "${FILE}" --hostname "${HN}" >/dev/null &&
  virt-customize -q -a "${FILE}" --run-command "echo -n > /etc/machine-id" >/dev/null
msg_ok "Customized image with Docker and Dockge successfully"
...
msg_info "Creating a Docker VM with Dockge"
qm create $VMID -agent 1${MACHINE} -tablet 0 -localtime 1 -bios ovmf${CPU_TYPE} -cores $CORE_COUNT -memory $RAM_SIZE \
  -name $HN -tags community-script,dockge -net0 virtio,bridge=$BRG,macaddr=$MAC$VLAN$MTU -onboot 1 -ostype l26 -scsihw virtio-scsi-pci
...
msg_ok "Created a Docker VM with Dockge ${CL}${BL}(${HN})"
